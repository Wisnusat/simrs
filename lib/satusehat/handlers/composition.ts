...
export const compositionHandler: SyncHandler = async (supabase, fhir, job) => {
  const { data: resume, error } = await supabase
    ...
    .select(`
      id, resume_date, chief_complaint, history_of_illness, physical_examination, summary, follow_up_plan, ss_composition_id,
      patients:patient_id ( id, full_name ),
      encounters:encounter_id ( id, ss_encounter_id ),
      author:authored_by ( id )
    `)
    .eq('id', job.local_id)
    .single()
  ...
  const existingId = (resume as any).ss_composition_id as string | null
  ...
  const res = existingId
    ? await fhir.put(`/Composition/${existingId}`, { ...payload, id: existingId })
    : await fhir.post('/Composition', payload)

  await logSync(supabase, {
    resource_type: 'Composition', local_id: resume.id, ss_resource_id: res.body?.id ?? existingId ?? undefined,
  ...
  const { error: updateErr } = await supabase.from('medical_resumes').update({
    ss_composition_id: res.body?.id ?? existingId,
